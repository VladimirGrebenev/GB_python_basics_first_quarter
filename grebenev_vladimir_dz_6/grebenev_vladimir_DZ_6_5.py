...
# # Домашняя работа к уроку №6 задание 5(3++)
...
users = []
with open(argv[1], encoding='utf-8') as f_users:
    for line in f_users:
        name = (line.replace('\n', '').split(','))
        users.append(name)

hobby = []
with open(argv[2], encoding='utf-8') as f_hobby:
    for line in f_hobby:
        h_list = (line.replace('\n', '').split(','))
        hobby.append(h_list)

with open(argv[3], 'w', encoding='utf-8') as f_dict:
